...
//! Project version number for FSWeakGCDTimer.
FOUNDATION_EXPORT double FSWeakGCDTimerVersionNumber;

//! Project version string for FSWeakGCDTimer.
FOUNDATION_EXPORT const unsigned char FSWeakGCDTimerVersionString[];

// In this header, you should import all the public headers of your framework using statements like #import <FSWeakGCDTimer/PublicHeader.h>

/**
 `FSWeakGCDTimer` behaves similar to an `NSTimer` but doesn't retain the target.
 This timer is implemented using GCD, so you can schedule and unschedule it on arbitrary queues (unlike regular NSTimers!)
 */
@interface FSWeakGCDTimer : NSObject
 ...
 * @param repeats if `YES`, `selector` will be invoked on `target` until the `FSWeakGCDTimer` object is deallocated or until you call `invalidate`. If `NO`, it will only be invoked once.
...
/**
 * Creates an `FSWeakGCDTimer` object and schedules it to start ticking inmediately.
 */
 ...
 * @note `FSWeakGCDTimer` won't invoke the `selector` on `target` again after calling this method.
